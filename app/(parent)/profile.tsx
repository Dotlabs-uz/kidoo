...
function ChildCard({ child, tasks }: { child: Child; tasks: any[] }) {
  const done = tasks.filter(t => t.status === 'done' && t.child_id === child.id).length;
  const pending = tasks.filter(t => t.status === 'pending' && t.child_id === child.id).length;
    ...
    <View style={styles.childCard}>
      ...
      <View style={styles.childStats}>
        ...
        <View style={[styles.childStatPill, { backgroundColor: '#FFF4D1' }]}>
          <Text style={[styles.childStatText, { color: '#8A6D14' }]}>🔥 {child.streak}</Text>
        </View>
      </View>
    </View>
  );
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.sectionHeader}>
          <View>
            ...
            <Text style={styles.sectionSub}>{children.length} {children.length === 1 ? 'ребёнок' : 'детей'} в семье</Text>
          </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <View style={styles.avatarGrid}>
              {AVATARS.map(av => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.avatarBtn,
                    newAvatar === av.id && { borderColor: av.color, backgroundColor: av.color + '18' },
                  ]}
                >
            ...
            <View style={styles.modalActions}>
              <Btn label="Отмена" variant="ghost" onPress={() => setShowModal(false)} style={{ flex: 1 }} />
...
const styles = StyleSheet.create({
  ...
  roleBadge: {
    backgroundColor: Colors.primarySoft, paddingHorizontal: 14, paddingVertical: 4,
  ...
  codeBlock: {
    width: '100%', backgroundColor: Colors.bg, borderRadius: 18, padding: 16, alignItems: 'center',
    borderWidth: 2, borderColor: Colors.line,
  },
  codeLabel: { fontSize: 11, fontWeight: '700', color: Colors.ink3, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 },
  codeValue: { fontSize: 32, fontWeight: '900', letterSpacing: 8, color: Colors.primary },
  copyBtn: {
  ...
  childCard: {
    backgroundColor: '#fff', borderRadius: 22, borderWidth: 2, borderColor: Colors.line,
  ...
  childStatPill: {
    backgroundColor: '#DFFBE9', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999,
  ...
  modalHandle: {
    width: 40, height: 4, borderRadius: 999, backgroundColor: Colors.line,
  ...
  avatarBtn: {
    width: '30%', padding: 12, borderRadius: 18,
    borderWidth: 2, borderColor: Colors.line, backgroundColor: '#fff',
